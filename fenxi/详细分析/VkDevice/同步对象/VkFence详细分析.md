...
## 目录
1. [栅栏是什么？用生活例子理解](#栅栏是什么用生活例子理解)
2. [VkFence 概述](#vkfence-概述)
3. [栅栏的作用与重要性](#栅栏的作用与重要性)
4. [栅栏 vs 信号量 vs 事件](#栅栏-vs-信号量-vs-事件)
5. [栅栏的状态](#栅栏的状态)
6. [栅栏的创建](#栅栏的创建)
7. [栅栏的使用](#栅栏的使用)
8. [栅栏等待](#栅栏等待)
...
**栅栏（Fence）就像红绿灯**：
- 🔴 **红灯（未发出信号）** = GPU 工作还没完成，CPU 必须等待
- 🟢 **绿灯（已发出信号）** = GPU 工作完成了，CPU 可以继续

### 🏗️ 更具体的例子：建筑工地

想象你是一个项目经理（CPU），派工人（GPU）去盖房子：
...
**关键点**：
- 项目经理（CPU）必须等待工人（GPU）完成工作
- 栅栏就是那个"等待标志"，告诉项目经理"工作完成了吗？"
- 只有栅栏显示"完成"时，项目经理才能继续下一步
...
// 你必须等待栅栏变成"已完成"，才能重用命令缓冲区
vkWaitForFences(device, 1, &fence, VK_TRUE, UINT64_MAX);
// 现在可以安全地重用命令缓冲区了
```

**为什么必须等待？**
- 如果不等GPU完成就重用命令缓冲区，会导致数据竞争
- 就像不等外卖送达就开始吃，会出问题
...
```mermaid
graph LR
    A[🔴 未发出信号<br/>UNSIGNALED<br/>GPU工作未完成] -->|GPU完成工作| B[🟢 已发出信号<br/>SIGNALED<br/>GPU工作已完成]
    B -->|重置栅栏| A
...
**状态说明**：
- **🔴 未发出信号（UNSIGNALED）**：GPU 还在工作，CPU 必须等待
- **🟢 已发出信号（SIGNALED）**：GPU 工作完成，CPU 可以继续

### 🏃 完整的工作流程

```mermaid
sequenceDiagram
    participant 你 as 你（CPU）
    participant GPU as GPU
    participant 栅栏 as 栅栏
...
|------|--------------|-------------------|
| **谁使用** | CPU 等待 GPU | GPU 内部同步 |
| **比喻** | 项目经理等工人完成 | 工人之间的协调 |
| **例子** | 你等外卖送达 | 外卖员之间协调送餐顺序 |
| **查询** | ✅ CPU可以查询 | ❌ CPU不能查询 |
| **等待** | ✅ CPU可以等待 | ❌ CPU不能等待 |
...
帧1: [🟢完成] → 可以重用 → [🔴重置] → [提交] → [执行中...]
帧2: [执行中...] → [🟢完成] → 可以重用 → [🔴重置] → [提交]
帧3: [提交] → [执行中...] → [🟢完成] → 可以重用 → [🔴重置]
...
**关键点**：
- 每帧都有自己的栅栏
- 必须等待当前帧的栅栏完成，才能重用该帧的资源
- 这样可以实现多帧并发，提高性能

### ✅ 总结：栅栏就是什么？

**一句话总结**：栅栏（Fence）就是**CPU等待GPU完成工作的"红绿灯"**，确保GPU真的完成了工作，CPU才能继续。

**三个关键词**：
1. **CPU-GPU同步** - CPU等待GPU完成
2. **状态标志** - 显示GPU工作是否完成
3. **资源安全** - 确保可以安全重用资源

**记住这个公式**：
```
CPU提交任务 → GPU执行 → 栅栏等待 → GPU完成 → 栅栏信号 → CPU继续
```
